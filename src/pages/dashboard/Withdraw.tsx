...
export default function WithdrawPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/user/stats').then(r => r.json()).then(data => setBalance(data.balance || 0));
  }, [user]);
  ...
  const handleWithdrawInitiate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success('Confirmation code sent to your encrypted email node.');
      } else {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold mb-2 tracking-tight uppercase">Capital Extraction</h1>
        <p className="text-gray-500 text-sm font-medium">Verify node credentials to authorize external liquidity transfer.</p>
      </div>
        ...
        {step === 'details' ? (
          <div className="space-y-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/5 rounded-xl py-4 px-4 focus:ring-1 focus:ring-brand-teal outline-none transition-all font-black text-lg"
                    placeholder="0.00"
                  />
                  <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                    {[25, 50, 100].map(p => (
                      <button 
                        key={p}
                        onClick={() => setAmount(((balance * p) / 100).toString())}
                        className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-brand-teal hover:text-slate-900 text-[10px] font-black transition-all uppercase"
                      >
                        {p}%
                      </button>
                    ))}
              ...
              {scanning && (
                <div id="reader" className="w-full rounded-xl overflow-hidden border border-brand-teal/50" />
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/5 rounded-xl py-4 pl-12 pr-4 focus:ring-1 focus:ring-brand-teal outline-none transition-all tracking-[1.5em] font-black"
                    placeholder="••••"
                  />
                </div>
              </div>
          ...
          <div className="space-y-8 py-4">
             <div className="text-center space-y-4">
                ...
                <div>
                   ...
                   <p className="text-xs text-gray-500 font-medium max-w-xs mx-auto mt-2 leading-relaxed">
             ...
             <div className="space-y-6">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-black/40 border border-white/5 rounded-xl py-6 px-4 focus:ring-1 focus:ring-brand-gold outline-none transition-all text-center font-black text-4xl tracking-[0.5em] text-brand-gold"
                    placeholder="000000"
                  />
                </div>
                ...
                <div className="flex gap-4">
                   <button 
                     ...
                     className="flex-1 py-4 bg-white/5 border border-white/10 rounded-xl font-black text-[10px] uppercase tracking-widest text-gray-400 hover:text-white transition-all"
                   >
                     Abort
                   </button>
                   <button 
