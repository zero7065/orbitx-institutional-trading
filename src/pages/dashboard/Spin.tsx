...
import { RotateCcw, Award, Gift, Star, Zap, Coins, Loader2 } from 'lucide-react';
...
const FALLBACK_PRIZES = [
  ...
  { id: '4', label: '250 USDT', reward: 250, icon: Gift, color: '#8B5CF6' },
  { id: '5', label: '500 USDT', reward: 500, icon: Star, color: '#FFD700' },
  { id: '6', label: '25 USDT', reward: 25, icon: Coins, color: '#10B981' },
];
...
export default function SpinPage() {
  ...
  useEffect(() => {
    fetch('/api/spin/prizes')
      .then(r => r.json())
      .then(data => setPrizes(data.length > 0 ? data : FALLBACK_PRIZES))
      .catch(() => setPrizes(FALLBACK_PRIZES))
  ...
  const handleSpin = async () => {
    ...
    try {
      const res = await fetch('/api/spin/spin', { method: 'POST', headers: { 'Content-Type': 'application/json' } });
      if (!res.ok) { const data = await res.json(); toast.error(data.error || 'Spin failed'); setSpinning(false); return; }
      const data = await res.json();
      const prize = data.prize;

      const prizeIndex = prizes.findIndex((p: any) => p.id === prize.id);
      const idx = prizeIndex >= 0 ? prizeIndex : Math.floor(Math.random() * prizes.length);
      const spins = 5 + Math.random() * 3;
      const prizeAngle = (360 / prizes.length) * idx;
      ...
      setTimeout(() => {
        ...
        setStats(prev => ({
          wins: prev.wins + 1,
          maxWin: Math.max(prev.maxWin, prize.rewardAmount || prize.reward),
          lastSpin: new Date()
        }));
        ...
        toast.success(`You won ${prize.label}! Added to your balance.`);
      }, 5000);
    } catch {
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: `linear-gradient(${angle % 90}deg, ${prize.color || '#00D1FF'}40, ${prize.color || '#00D1FF'}20)`,
                    transform: 'skewY(-67.5deg)',
                    borderRight: `2px solid ${prize.color || '#00D1FF'}40`
                  }}
                />
                ...
                >
                  <span className="text-[10px] font-black uppercase tracking-wider"
                    style={{ color: prize.color || '#00D1FF', textShadow: `0 0 10px ${prize.color || '#00D1FF'}` }}
                  >
                    {prize.label}
                  </span>
                </div>
      ...
      <AnimatePresence>
        {result && (
          ...
          >
            ...
            >
              <Zap size={64} className="mx-auto mb-4 text-brand-gold" />
              ...
              <p className="text-5xl font-black text-brand-teal mb-4">{result.label}</p>
               <p className="text-2xl font-bold text-brand-gold mb-4">+${(result.rewardAmount || result.reward || 0).toFixed(2)}</p>
